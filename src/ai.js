...
async function executeTool(guild, actorId, name, args) {
  ...
  const management = new Set(["create_role", "create_channel"]);
  ...
  const cooldowns = {
    ...
    create_role: 10, create_channel: 10, remember: 3
  };
  ...
  if (name === "create_channel") {
    ...
    const channel = await guild.channels.create({
      name: String(args.name).trim().toLowerCase().replace(/\s+/g, "-").slice(0, 100),
